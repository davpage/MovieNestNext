...
export default function MovieList({ movies, title, loading, onMovieClick }) {
    return (
        <div className="max-[850px]:w-full h-full min-[850px]:min-w-[350px] overflow-y-auto bg-black/50 backdrop-blur-lg border border-red-900 p-4 shadow-lg">
            ...
            <div className="space-y-4">
                {movies.map((movie, index) => (
                    <div
                        key={index}
                        className="flex items-center gap-4 p-2 border border-gray-400 text-white hover:text-gray-700 rounded-lg bg-white/10 backdrop-blur-lg cursor-pointer hover:bg-gray-100 transition"
